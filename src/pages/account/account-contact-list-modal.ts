...
@Component({
    templateUrl: 'account-contact-list-modal.html'
})
export class AccountContactListModalPage extends BasePage {
  ...
  private loadPotentials(recordId: any): Promise<any> {
      return this.userService.getCurrentUserSessionId().then(
          sessionDto => {
            return this.accountService.getRelatedContacts(sessionDto, recordId)
            .then(
                data => {
