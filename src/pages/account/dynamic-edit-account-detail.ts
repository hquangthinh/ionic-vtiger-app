...
@Component({
  selector: 'page-dynamic-edit-account-detail',
  templateUrl: 'dynamic-edit-account-detail.html'
})
export class DynamicEditAccountDetailPage extends BasePage {
  ...
  private loadDataForEdit(loader: Loading) {
    this.userService.getCurrentUserSessionId().then(
      sessionDto => {
        ...
        let obFormMeta = this.moduleService.getAccountModuleFormSchemaFromCache();

        Observable.zip(obModuleMeta, obUsers, obFormMeta).subscribe(dataList => {
            let formMeta = dataList[0];
            let userData = dataList[1];
            this.formSchema = dataList[2];
